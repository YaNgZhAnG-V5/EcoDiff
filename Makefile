...
cfg=cfg
visual:
	$(PYTHON) ./scripts/train.py --save_dir $(RESULTS_DIR) --cfg $(CONFIG_DIR)/${cfg}.yaml
...
gen:
	$(PYTHON) ./scripts/utils/hyperparameter_tuning.py --task gen

run:
	$(PYTHON) ./scripts/utils/hyperparameter_tuning.py --task run
