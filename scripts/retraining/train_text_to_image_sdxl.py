...
from diffusers.utils import check_min_version, is_wandb_available, SAFETENSORS_WEIGHTS_NAME, WEIGHTS_NAME
...
def main(args):
    ...
    unet.requires_grad_(True) ###### Added require grad ######
    unet.train()
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                ...
                for i, model in enumerate(models):
                    ###### custom saving ######
                    if args.pruned_unet_model_path is not None:
                        model.half().save_pretrained(os.path.join(output_dir, "unet"), safe_serialization=True)
        ...
        def load_model_hook(models, input_dir):
            ...
            for _ in range(len(models)):
                ...
                ###### custom loading ######
                if args.pruned_unet_model_path is not None:
                    filepath = os.path.join(input_dir, "unet", SAFETENSORS_WEIGHTS_NAME)
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("text2image-full-finetune-sdxl", config=vars(args))
                ...
                if accelerator.distributed_type == DistributedType.DEEPSPEED or accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        if args.pruned_unet_model_path is not None:
                            ...
                            with open(
                                os.path.join(
                                    "/".join(args.pruned_unet_model_path.split("/")[:-1]), f"finetune_{file_name}"
                                ),
                                "wb",
                            ) as f:
                                pickle.dump(unet_state_dict, f)
                            unet.to(torch.float32)
                        ###### save the pruned unet model ######
