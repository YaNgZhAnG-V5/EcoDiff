...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to pretrained VAE model with better numerical stability. More details: "
            "https://github.com/huggingface/diffusers/pull/4038."
        ),
    )
    parser.add_argument(
        "--pruned_unet_model_path",
        type=str,
        default=None,
        help="Path to pruned unet model.",
    )
    parser.add_argument(
        "--revision",
        type=str,
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "The prediction_type that shall be used for training. Choose between 'epsilon' or"
            " 'v_prediction' or leave `None`. If left to `None` the default prediction type of the scheduler:"
...
def main(args):
    ...
    # custom unet ##############
    if args.pruned_unet_model_path is not None:
        with open(args.pruned_unet_model_path, "rb") as f:
            unet = pickle.load(f)
    ...
    # For mixed precision training we cast all non-trainable weights (vae, non-lora text_encoder
    # and non-lora unet) to half-precision
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                # DeepSpeed requires saving weights on every device; saving weights only on the main process
                # would cause issues.
                if accelerator.distributed_type == DistributedType.DEEPSPEED or accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        # save the pruned unet model ######
                        if args.pruned_unet_model_path is not None:
                            unet = unwrap_model(unet)
                            lora_state_dict = convert_state_dict_to_diffusers(get_peft_model_state_dict(unet))
                            file_name = args.pruned_unet_model_path.split("/")[-1]
                            with open(
                                os.path.join(
                                    "/".join(args.pruned_unet_model_path.split("/")[:-1]), f"lora_{file_name}"
                                ),
                                "wb",
                            ) as f:
